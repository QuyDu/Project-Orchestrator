...
catalog, schemas, workflows, prompts, agents, blueprint contract, and Azure discovery scaffold. The
discovery report covers chat models, Speech readiness, and sanitized image-model, quota, and existing-
deployment evidence without creating resources or persisting Azure resource identifiers.
...
```text
/project-visual-storytelling create --output-type diorama --request "Create a photorealistic handcrafted miniature of this project's architecture" --external-processing-approved true
```

The `create` command is the normal Copilot-style path for whiteboards and dioramas. From one bounded natural-language request it refreshes Project Understanding, prioritizes meaningful project capabilities over folder names, builds evidence-backed labels and captions, renders with an eligible provider, verifies the result, and returns the PNG candidate plus specification and alt text. The lower-level `preflight`, `render`, and `verify` commands remain available for controlled workflows.

Diorama preflight automatically selects an architectural treatment for verified components, topology, platforms, services, or boundaries; otherwise it uses a conceptual treatment. The visual standard is a macro product photograph of a handcrafted miniature exhibit: tactile cardboard, foam, clay, wood, paper, warm studio lighting, physical depth, real shadows, one central sculpted metaphor, supporting shadow-box panels, and fewer larger labels. Whiteboards use a front-facing photographed foam-board display with dimensional cut-paper panels, natural marker lettering, restrained semantic accents, small project-specific props, and strong information hierarchy. Reference images guide those medium-level traits only; their subjects, wording, branding, signatures, dates, source lines, and exact composition are never copied or submitted.

A rendered whiteboard or diorama requires an approved Azure Government bitmap image generator. HTML/CSS/SVG screenshots and flat faux-isometric compositions do not qualify. When no qualified provider is available, `create` fails clearly and does not claim a PNG; use the explicit `whiteboard-specification` or `diorama-specification` mode only when specification and alt text are the intended output. Every rendered visual uses only the validated User Personalization signature and current profile timezone date. The canonical spelling is `diorama`.

No separately installed rendering application is required. The packaged Node workflow includes Azure OpenAI GPT Image and Microsoft MAI adapters, validates them against fresh `azure-discovery` evidence and Azure Government endpoints, and authenticates with Microsoft Entra. Use `doctor --project .` to inspect provider readiness. Automated verification returns `requires-review`; the PNG remains a draft until photographic fidelity, shadows, labels, captions, signature, date, and evidence mappings are inspected at full size.

The render plan may select `auto`, `azure-openai`, or `mai-image`. With `--external-processing-approved true` on the current command, `auto` prefers a discovery-selected, generally available Azure OpenAI GPT Image 2 deployment and then a qualified Microsoft MAI-Image deployment. Without that approval, or when no provider supports the requested dimensions, rendering fails closed. Configuration and prior approvals never authorize prompt submission. Authentication uses Microsoft Entra; API keys, web grounding, and reference-pixel submission are unsupported. Output remains `<visual>-azure-openai-candidate.png` or `<visual>-mai-candidate.png` and cannot become final until exact text, signature, date, physical style, and evidence checks pass.
...
| `node .\pso.mjs plan --root PATH --intent TEXT` | Create a read-only workflow plan that matches installed skills, expands declared prerequisites, and records a terminal handoff |
