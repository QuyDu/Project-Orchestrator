...
- Run `node .github/skills/project-visual-storytelling/scripts/project-visual-storytelling.mjs doctor --project .` before promising a rendered PNG. It reports the optional MAI-Image and Blender adapters. Blender is operator-installed software; this skill never installs, downloads, or deploys a renderer.
...
- Use the packaged Blender adapter for local physically based rendering. It accepts only the bounded render-plan data contract, starts Blender with factory settings, uses procedural assets, and writes only to the preflight-assigned run directory.
...
- Treat MAI output as `<visual>-mai-candidate.png`, not the final labeled visual. It may supply the photorealistic raster stage, but deterministic attribution and exact-text qualification remain required before delivery. Current Azure Government discovery does not confirm MAI-Image, so the adapter normally remains unavailable and `auto` falls back to Blender.
...
10. Before creating or promising `diorama.png`, add `## Renderer Qualification` to the specification. Record renderer class, tool or capability, local or approved external-processing boundary, and validation evidence. For `physically-based-3d`, record geometry, perspective camera, material model, lights, and cast/contact shadows. For `bitmap-generation`, record the approved raster capability and whether reference pixels were supplied. An HTML/CSS/SVG screenshot does not qualify. A planned optional PNG is not a promise of delivery.
11. For a render, create `render-plan.json` in `destination.directory` using `schemas/project-visual-scene.schema.json`. Bind it to the preflight repository digest and current profile signature/date; select `auto`, `mai-image`, or `blender-cycles`; include a bounded image prompt; use one to twelve bounded, evidence-mapped elements; and include no scripts, URLs, external assets, or unapproved paths.
...
16. Run `render --project . --run-id <assigned-run-id>`. The runtime revalidates the context, current profile, source digest, plan, and current timezone date. `auto` prefers a qualified MAI adapter only when the command also includes `--external-processing-approved true`; otherwise it uses Blender. An explicit unavailable provider and every specification-only run fail closed.
...
15. Save the required files under `destination.directory` from preflight. Whiteboard runs use `whiteboard-specification.md`, `whiteboard-alt-text.md`, and optional `whiteboard.png`; diorama runs use `diorama-specification.md`, `diorama-alt-text.md`, and optional `diorama.png`. Local renders also retain `render-context.json`, `render-plan.json`, and `renderer-qualification.json` as validation evidence.
...
- Local renderer qualification proves Blender Cycles, modeled geometry, at least three materials, at least two lights, a perspective camera, plan-digest binding, intended dimensions, pixel variation, and the exact planned text record before human review.
...
- If Blender is missing, Cycles is unavailable, the render exceeds its bounded execution time, or automated qualification fails, remove incomplete renderer outputs and preserve the specification-and-alt-text fallback.
- If MAI configuration, Government discovery, Entra authentication, deployment access, content filtering, response validation, or explicit external-processing approval fails, remove incomplete MAI outputs and either use Blender under `auto` or preserve the specification fallback for an explicit `mai-image` request.
...
- Require `--external-processing-approved true` on the current render invocation before sending a project-derived prompt to MAI-Image. Stored configuration, prior approval, or `auto` selection alone is insufficient.
...
- Use fresh `azure-discovery` output to qualify MAI-Image availability in Azure Government. Its absence or failure does not block local Blender or specification-only output.
...
- Select `auto` to prefer a discovery-confirmed Government MAI deployment for a photorealistic candidate after explicit external-processing approval, then retain Blender as the deterministic final renderer and fallback.
